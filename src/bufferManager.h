...
class BufferManager{
    ...
    deque<pair<Page, bool>> pages; //bool to check whether the page in deque has become dirty ( 0=clean, 1=dirty )
    ...
    Page getPageNotInCatalogue(string tableName, int pageIndex, int columnCount, int rowCount);
    ...
    void writeToCache(string tableName, int pageIndex, vector<vector<int> > &rows, int rowCount);
